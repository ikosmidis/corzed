...
# Table 2
`dyslexia_simulation_group1.rda`, ..., `dyslexia_simulation_group5.rda` are the outputs of [`dyslexia_simulation.R`](`r paste(code_path, "dyslexia_simulation.R", sep = "/")`) in `r code_path`, which replicates the simulation study described in Example 1.1 of @dicaterina+kosmidis:2017
```{r}
results <- NULL
for (j in c(1:5)) {
    load(paste(results_path, paste0("dyslexia_simulation_group", j, ".rda"), sep = "/"))
...
rs_coverage <- results %>%
    ...
    summarize(coverage = round(mean(cover) * 100, 1)) %>%
    as.data.frame() %>%
    reshape(idvar = c("level", "parameter"), v.names = "coverage",
            timevar = "statistic",
            direction = "wide")

rs_coverage %>% select(level, parameter, coverage.ml, coverage.br, coverage.ml_cor, coverage.br_cor)
rs_coverage %>% select(level, parameter, coverage.ml_stud, coverage.br_stud, coverage.ml_cor_stud, coverage.br_cor_stud)
```
...
quantiles_ml <- dyslexia_bootstrap(rs_beta_ml, R = 100, ncores = 4)$quantiles
quantiles_br <- dyslexia_bootstrap(rs_beta_br, R = 100, ncores = 4)$quantiles

rs_cor_ml_stud_cis <- waldi_confint(rs_beta_ml, adjust = TRUE,
                                    quantiles = quantiles_ml$zstat_cor[, c("0.025", "0.975")])
rs_cor_br_stud_cis <- waldi_confint(rs_beta_br, adjust = TRUE,
                                    quantiles = quantiles_br$zstat_cor[, c("0.025", "0.975")])
rs_ml_stud_cis <- waldi_confint(rs_beta_ml, adjust = FALSE,
                                quantiles = quantiles_ml$zstat[, c("0.025", "0.975")])
rs_br_stud_cis <- waldi_confint(rs_beta_br, adjust = FALSE,

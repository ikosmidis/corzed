...
set.seed(123)
quantiles_ml <- dyslexia_bootstrap(rs_beta_ml, R = 100, ncores = 1)$quantiles
quantiles_br <- dyslexia_bootstrap(rs_beta_br, R = 100, ncores = 1)$quantiles
...
rs_coverage %>% filter(statistic %in% c("ml_stud", "br_stud")) %>%
    select(statistic, parameter, coverage.90, coverage.95, coverage.99)
...
```{r}
system.time({
    waldi_confint(rs_beta_ml, adjust = TRUE, parallel = FALSE)
})

simu_fun <- get_simulate_function(rs_beta)
...
system.time({
    stats <- boot(ReadingSkills$accuracy, statistic = stat,
                  R = 100, sim = "parametric", ran.gen = generate_dyslexia,

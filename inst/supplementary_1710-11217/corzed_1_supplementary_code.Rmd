...
knitr::opts_chunk$set(collapse = T, comment = "#", fig.width=7, fig.height=4, out.width="75%", dpi = 200)
options(width = 80)
```
...
rs_bc_estimates <- do.call("rbind", lapply(rs_summary_bc,
                                           function(z) z[, c("Estimate", "Std. Error")]))
...
table4 <- matrix(c(out1, out2), ncol = 7, byrow = TRUE,
                 dimnames = list(NULL, c("mle", "mcle", "wald_ml", "wald_mlc",
                                         "r", "rc", "wald_adjusted")))
round(table4, 4)
```
...
pval <- pval %>%
    ...
    mutate(test = dplyr::recode(test,
           ...
           statistic = factor(statistic,
                              levels = c("mle", "r", "cond", "scorec", "rc", "cor"),
                              ordered = TRUE)) %>%
  ...
  lims(y = c(85, 100)) +
    ...
    scale_linetype_manual(name = "", values = c(2, 1, 3, 4, 5),
                          labels = c(expression(italic(t)),
                                     ...
                                     "DL",
                                     expression(italic(r)[p]),
                                     expression(italic(r)^list("*")))) +
    theme_bw() +
    theme(text=element_text(size = 11)) +
    theme(legend.position = "none", panel.grid.major.y = element_blank(),
          panel.grid.minor.y = element_blank(),
...
table5 <- cbind(corzed_cis[-c(1, 5),],
                ...
                reshape(idvar = c("parameter"), v.names = "coverage.cor",
